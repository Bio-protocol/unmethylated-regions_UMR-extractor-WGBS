...
bedGraph <- read_tsv(paste0("BSMAP_out.txt.100.CG.bed"), col_names = F,
                            cols(
                              X1 = col_character(),
                              X2 = col_integer(),
                              X3 = col_integer(),
...
bedGraph <- read_tsv(paste0("BSMAP_out.txt.100.CHG.bed"), col_names = F,
                     cols(
                       X1 = col_character(),
                       X2 = col_integer(),
                       X3 = col_integer(),
...
bedGraph <- read_tsv(paste0("BSMAP_out.txt.100.CHH.bed"), col_names = F,
                     cols(
                       X1 = col_character(),
                       X2 = col_integer(),
                       X3 = col_integer(),
...
write.table(mC_domains_freq, "mC_domains_freq.tsv", sep = "\t", quote = F, row.names = F, col.names = T)
...
write.table(mC_domains2, paste0("mC_domains",
...
write.table(UMT_only, paste0("UMTs.bed"), sep = "\t", quote = F, row.names = F, col.names = F)
...
write.table(ND_only, paste0("NDs.bed"), sep = "\t", quote = F, row.names = F, col.names = F)
...
write.table(NDs_data, paste0("mC_domains",
                                   "_cov_",coverage_filter_min,
                                   "_sites_",site_filter_min,
                                   "_MR_",MR_percent,
